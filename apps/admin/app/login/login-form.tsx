...
export default function LoginForm({ apiMode }: { apiMode: boolean }) {
      ...
      <div className="w-full max-w-md">
        ...
        <div className="w-full bg-white rounded-2xl border border-slate-200 p-6 sm:p-8">
          ...
          <form action={formAction} className="space-y-4">
            {apiMode && <div>
              <label htmlFor="agencySlug" className="block text-xs font-semibold text-slate-700 mb-2">Código de tu agencia</label>
              <input id="agencySlug" name="agencySlug" required maxLength={80} pattern="[a-z0-9]+(-[a-z0-9]+)*" autoCapitalize="none" autoCorrect="off" placeholder="mi-agencia" className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm" />
              <p className="text-xs text-slate-500 mt-2">Usa el código que te entregó el administrador de tu agencia.</p>
            </div>}
            <div>
